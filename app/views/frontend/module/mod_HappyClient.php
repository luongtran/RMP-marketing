 ...
 <h3 class="lined margin-20">
    <?php   $happyClients_intro = DB::table('page_module')
                   ...
                   ->first(); 
    if($happyClients_intro)                   
     echo $happyClients_intro->title; ?>
   </h3>
      <div style="margin-bottom: 10px;" class="b-clients">
        <?php   $happClients_content = DB::table('page_module')
                   ->leftjoin("pages","page_module.page_id","=","pages.id")
                   ->leftjoin("module","page_module.module_id","=","module.id")
                   ...
                   ->get(); 
    foreach($happClients_content as $listContent):?>
        <div class="client tooltips">
          <div class="tooltips-data">
            <?php echo $listContent->title;?>
          </div>
          <a href="#"><img alt="" src="<?php echo asset($listContent->path.'/'.$listContent->imageName);?>"></a>
        </div>
    <?php endforeach;?> 
  </div>
<!-- module reason Our Happy clients -->  
